        ...
        @if (Route::has('login'))
            ...
            <div class="min-h-screen flex flex-col">
                ...
                <div class="flex flex-1 flex-col items-center items-top bg-gray-100 dark:bg-gray-900 sm:items-center sm:pt-0">
                    @if($workouts->isEmpty())
                        <div class="mt-16 md:mt-20 px-4 md:px-20 text-2xl md:text-3xl text-center font-semibold text-pink-500">
                            Dang, you haven't added any workouts. Create one right away, so you can start in no time!
                        </div>
                    @endif
                    @if(!$workouts->isEmpty())
                        <div class="mt-12 md:mt-20 text-3xl md:text-5xl font-semibold text-center text-green-500">
                            MY WORKOUTS
                            </div>
                        <div class="mt-4 mb-4 md:mt-12 md:mb-12 overflow-hidden grid grid-cols-1 md:grid-cols-3">
                            @foreach($workouts as $workout)
                                <a href="/workouts/{{$workout->id}}">
                                    <div class="my-4 md:mx-4" x-data="{difficulty: '{{$workout->difficulty}}'}">
                    ...
                    @endif
                        <a href="{{ url('/workouts/new') }}" class="mt-8 mb-8 px-4 py-2 w-auto flex items-center justify-center rounded-md bg-green-500 text-xl text-white font-semibold hover:bg-green-600 focus:bg-green-700">New Workout</a>
                </div>
            </div>
